...
# function which gets the data points and K (number of required centroids)
...
# function which gets the current matrix, res - 0/1 which indicates if the C function failed,
# and bool which indicate if need to print the matrix if the matrix is valid
# return True if need to Exit
def check_output(matrix, res, to_print):
    if res == 0:
        if to_print:
            print_matrix(matrix)
            return True
...
def main():
    try:
        ...
        if K < 0 or K >= data.shape[0]:
            print("Invalid Input!")
            return
        # if the goal is jacobi, then we get a symmetric matrix as input
        if goal == 'jacobi':
        ...
        # if any other goal, then start running the algorithm and stop according to the specific goal
            ...
            if K == -1:  # if the program failed it returns -1;
                ...
                return
            T = T[:, :K]
            # running the kmeans++ algorithm on T
